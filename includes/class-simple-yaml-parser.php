<?php
/**
 * Simple YAML Parser for WAC Chat Funnels
 * 
 * Parser YAML simple y robusto que funciona sin dependencias externas
 * Específicamente diseñado para la estructura de funnels del plugin
 *
...
class WAC_Chat_Simple_YAML_Parser {
    ...
    public static function parse($yaml_content) {
        ...
        error_log('WAC Chat Funnels - Simple YAML Parser: parsing YAML');
        error_log('WAC Chat Funnels - YAML length: ' . strlen($yaml_content));
        
        try {
            $result = self::parse_yaml_content($yaml_content);
            error_log('WAC Chat Funnels - Simple YAML Parser result: ' . print_r($result, true));
            return $result;
        } catch (Exception $e) {
            error_log('WAC Chat Funnels - Simple YAML Parser error: ' . $e->getMessage());
        ...
        foreach ($lines as $line_num => $line) {
            ...
            if (strpos($trimmed, ':') !== false) {
                ...
                if (strpos($option_line, ':') !== false) {
                    ...
                    $option_data[trim($opt_key)] = trim($opt_value, '"\'');
                } else {
                    $option_data['label'] = trim($option_line, '"\'');
                }
